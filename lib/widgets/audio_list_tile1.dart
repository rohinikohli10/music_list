...
import 'package:music_list_app/screen/audio_screen.dart';
...
class _AudioListTileState extends State<AudioListTile>
  ...
  //   (index) => AudioPlayer(),
  // );
  // AudioPlayer get _player => audioPlayers[selectedPlayerIdx];
  int? selectedIndex;
  bool isplaying = false;

  @override
  void initState() {
    print("---------video player ka init print result");
    print(widget.player.processingState);
    print(widget.player.currentIndex);
    // selectedPlayerIdx = widget.index;
    super.initState();
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      //     children:
      //         //widget.audioExamples[widget.index]['song']
      //         widget.audioExamples.map((e) {
      //       return AudioSource.uri(
      //           Uri.parse(widget.audioExamples[widget.index]['song']));
      //     }).toList(),
      //   ),
      // );
      //await _player.setAudioSource(AudioSource.uri(Uri.parse(widget.mySong)));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListTile(
            ...
            builder: (context, snapshot) {
              ...
              if (processingState == ProcessingState.loading ||
                ...
                return IconButton(
                    ...
                    iconSize: 40.0,
                    onPressed: () async {
                      print("====current index chaeck kar player ka==========");
                      print(widget.player.currentIndex);
                      print(widget.player.sequenceState?.currentIndex);
          ...
          //PlayPauseClass(player: _player, index: widget.index),
          ),
    );
  }
}
...
//                 print(player.sequenceState?.currentIndex);
//                 await player.play();
//               });
//         } else if (processingState != ProcessingState.completed) {
//           return IconButton(
